...
export function chooseBestSum(t: number, k: number, ls: number[]): number | null {
  const dists = getDists(ls, t, k);
  return dists.length > 0 ? Math.max(...dists) : null;
}

function getDists(ls: number[], t: number, k: number, index = 0, dist = 0, dists: number[] = []): number[] {
  if (k == 0) {
    if (dist <= t) dists.push(dist);
    return dists;
  }

  for (let i = index; i < ls.length; i++) {
    getDists(ls, t, k-1, i+1, dist + ls[i], dists);
  }
